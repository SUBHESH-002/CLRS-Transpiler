...
class SemanticAnalyzer:
    ...
    def analyze(self, node):
        if isinstance(node, Program):
            ...
            if t is None: raise SemanticError(f"Undefined variable: {node.name}")
            ...
            if self.get_var_type(node.array) is None: raise SemanticError(f"Undefined array: {node.array}")
